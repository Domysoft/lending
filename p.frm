...
Begin VB.Form Main 
   ...
   Begin VB.Frame homeFrame 
      ...
      Top             =   9000
      Width           =   8415
      Begin VB.Label load_back_up_label 
         BackStyle       =   0  'Transparent
         Caption         =   "Load Previous Back Up"
         BeginProperty Font 
            Name            =   "Catriel"
            Size            =   9.75
            Charset         =   0
   ...
   Begin VB.Image Image13 
      ...
      Width           =   330
   End
   Begin VB.Label system_version 
      BackStyle       =   0  'Transparent
      Caption         =   "V 1.01.01.12"
      BeginProperty Font 
         Name            =   "Catriel"
         Size            =   9.75
         Charset         =   0
...
MsgBox "The following are the relase number(s) with ledger that has not been approved." & vbNewLine & holder.Text, vbInformation
...
MsgBox "Client's Ledger can't be change due to previous transactions.", vbInformation
End Sub
...
rs.Open "SELECT ledger,d_c,area_name,release_id, edit_remarks, balance,release_number, date_paid FROM " & LCase(newArea(0)) & newArea(1) & " where date_paid = '" & newdate(0) & "/" & newdate(1) & "/" & newdate(2) & "' order by ledger", conn

'MsgBox "SELECT ledger,d_c,area_name,release_id, edit_remarks, balance,release_number, date_paid FROM " & LCase(newArea(0)) & newArea(1) & " where date_paid = '" & newdate(0) & "/" & newdate(1) & "/" & newdate(2) & "' order by ledger"
...
viewRemittanceGrid.TextMatrix(b, 9) = content(4)
...
help_grid.ColWidth(2) = 3500
help_grid.ColWidth(3) = 4000
...
help_grid.TextMatrix(10, 0) = "10"
    ...
    help_grid.TextMatrix(11, 2) = "print transaction?"
    help_grid.TextMatrix(11, 3) = "edit payment transaction?"
    
help_grid.TextMatrix(12, 0) = "11"
    help_grid.TextMatrix(12, 1) = "Print Preview (P)"
    help_grid.TextMatrix(12, 2) = "view pass due accounts?"
    help_grid.TextMatrix(12, 3) = "view master list?"
    help_grid.TextMatrix(13, 2) = "print master list?"
    help_grid.TextMatrix(13, 3) = "print pass due list?"

help_grid.TextMatrix(14, 0) = "12"
    help_grid.TextMatrix(14, 1) = "Developers (D)"
    
help_grid.TextMatrix(15, 0) = "13"
    help_grid.TextMatrix(15, 1) = "System Version"
...
ElseIf help_grid.Row = 15 And help_grid.Col = 1 Then
MsgBox "A text located at the top of Customer Search. Click the version text to view System Log.", vbInformation
...
query = "select balance,release_number, maturity, days_left from loan_release where ledger= '" & ledger_1(0) & "' order by release_number desc limit 1"
...
' changes made due to bal - 11-21-11
...
query = "INSERT INTO loan_release (date_release, ledger, percent,  balance, amount, area_name, client_name) values" _
& "('" & date_new_release.Value & "', '" & ledger_1(0) & "','" & percent_new_release.Text & "','" & (Val(Amount_new_release.Text) * Val(percent_new_release.Text)) & "','" & Val(Amount_new_release.Text) * Val(percent_new_release.Text) & "','" & Trim(ledger_1(3)) & "','" & Trim(ledger_1(1)) & ", " & Trim(ledger_1(2)) & "')"


conn.Execute query

query = "update client set balance = '" & (Val(Amount_new_release.Text) * Val(percent_new_release.Text)) & "' where ledger = '" & ledger_1(0) & "'"
    conn.Execute query
...
system_version.ForeColor = &HC0C0C0
If active = "help" Then
help1.Visible = False
Else
help1.Visible = True
...
Private Sub load_back_up_label_Click()
...
MsgBox "Only Admin can do this task!", vbExclamation
Else

ans = (MsgBox("Loading back up is NOT RECOMENDED." & vbNewLine & vbNewLine & "Are you sure you want to load back up?", vbYesNo))
If ans = 7 Then
Exit Sub
End If
...
Private Sub locker_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
locker.ForeColor = &HFF&
End Sub
...
Private Sub percent_new_release_GotFocus()
percent_new_release.Text = ""
End Sub
...
rs.MoveNext
Loop
PrintPreviewGrid.Col = 1
PrintPreviewGrid.Sort = 1
...
MsgBox "Please select Area first.", vbExclamation
Exit Sub
End If
...
System_log.Text1.Text = RichTextBox1.Text
Main.Enabled = False
System_log.Show
End If
End Sub
...
spliter = Split(COpLname.Text & ", no", ",")

If Trim(spliter(1)) = "no" Then
MsgBox "Wrong name format." & vbNewLine & "Use this format: 'Surename, Firstname Middle Initial'", vbExclamation
Exit Sub
End If
...
If content(16) = "" Then
remAmount.Text = "0"
MsgBox "Release in this person was not yet approve." & vbNewLine & "Person remittance's amount is forced to zero but total remittance is still incrementing.", vbCritical
End If

If content(10) = "Cancelled" Then
remAmount.Text = "0"
MsgBox "Last release in this person was Cancelled." & vbNewLine & "Person remittance's amount is forced to zero but total remittance is still incrementing.", vbCritical
End If
...
MsgBox "Area Name/Location can't be change due to previous transactions.", vbInformation
End Sub
...
MsgBox "Warning! A release has not been approve on this area!" & vbNewLine & "Person(s) with unapprove release will be automatically Php 0.00 on its daily remittance.", vbCritical
End If

rs.Open "select ledger from client where area_name like '" & SelectAreaRemittance.Text & "%' order by ledger ", conn
...
Private Sub SelectAreaRemittance_KeyDown(KeyCode As Integer, Shift As Integer)
SelectAreaRemittance.Locked = True
End Sub

Private Sub SelectAreaRemittance_KeyUp(KeyCode As Integer, Shift As Integer)
SelectAreaRemittance.Locked = False
End Sub
...
Private Sub SelectAreaViewR_KeyDown(KeyCode As Integer, Shift As Integer)
SelectAreaViewR.Locked = True
End Sub

Private Sub SelectAreaViewR_KeyUp(KeyCode As Integer, Shift As Integer)
SelectAreaViewR.Locked = False
End Sub

Private Sub system_version_Click()
RichTextBox1.LoadFile App.Path & "\log file.txt"
End Sub

Private Sub system_version_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
system_version.ForeColor = &H8080FF
End Sub
...
Private Sub UserDesigntion_KeyDown(KeyCode As Integer, Shift As Integer)
UserDesigntion.Locked = True
End Sub

Private Sub UserDesigntion_KeyUp(KeyCode As Integer, Shift As Integer)
UserDesigntion.Locked = False
End Sub
...
conn.Execute query
If UserOptask.Text = "Add User" Then
MsgBox "New user was successfully added!", vbInformation
End If

If UserOptask.Text = "Edit User" Then
MsgBox "User updated!", vbInformation
End If
...
index_num = UserOpSelName.ListIndex

UserOpName.Text = user_grid.TextMatrix(index_num + 1, 1)
UserOpUsername.Text = user_grid.TextMatrix(index_num + 1, 2)

UserDesigntion.Text = user_grid.TextMatrix(index_num + 1, 4)
userOp_blanck.Text = user_grid.TextMatrix(index_num + 1, 5)
End Sub

Private Sub UserOpSelName_KeyDown(KeyCode As Integer, Shift As Integer)
UserOpSelName.Locked = True
End Sub

Private Sub UserOpSelName_KeyUp(KeyCode As Integer, Shift As Integer)
UserOpSelName.Locked = False
End Sub
...
Private Sub UserOptask_KeyDown(KeyCode As Integer, Shift As Integer)
UserOptask.Locked = True
End Sub

Private Sub UserOptask_KeyUp(KeyCode As Integer, Shift As Integer)
UserOptask.Locked = False
End Sub
...
EditRemittance.notice.Text = viewRemittanceGrid.TextMatrix(viewRemittanceGrid.Row, 9)
...
rs.Open "SELECT date_release, release_number, date_approve, ledger FROM loan_release order by release_number", conn

Dim rel_counter  As Long
Dim rel_records(4) As String
Dim last_rec As String
Dim unapprove_counter As Integer
...
aa = 0
    For Each fld In rs.Fields
        rel_records(aa) = fld.Value
        aa = aa + 1
    Next
    
    If rel_records(2) = "" Then
            unapprove_counter = unapprove_counter + 1
            not_approved(unapprove_counter) = "Release Number : " & rel_records(1) & ", Ledger: " & rel_records(3) & "" & vbNewLine
            'MsgBox "release number : " & loan_counter + 1 & " Ledger: " & q_value(1)
    End If
    
last_rec = rel_records(0)
rel_counter = rel_counter + 1
rs.MoveNext
Loop

release_count.Caption = rel_counter
...
last_rel.Caption = "Last release was reformed " & last_rec & "."

' release counter ends


Exit Sub
' codes below will not be executed due to release number bug in home


' for release ---------------


rs.Open "SELECT date_approve,ledger FROM loan_release order by release_number", conn

'Dim not_approved(99999) As String
Dim q_value(2) As String
...
data_count = 0
For Each fld In rs.Fields
q_value(data_count) = fld.Value
data_count = data_count + 1
Next

If q_value(0) = "" Then
unapprove_counter = unapprove_counter + 1
not_approved(unapprove_counter) = "Release Number : " & loan_counter + 1 & ", Ledger: " & q_value(1) & "" & vbNewLine
'MsgBox "release number : " & loan_counter + 1 & " Ledger: " & q_value(1)
Else
last_value = q_value(0)
End If
...
load_back_up_label.Visible = True
...
load_back_up_label.Visible = False
